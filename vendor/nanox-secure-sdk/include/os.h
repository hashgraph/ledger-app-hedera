...
enum task_unsecure_id_e {
  TASK_PREVIOUS = 0x01DB0105,   // unreachable in task array
  TASK_BOLOS_INIT = 0x600dB010, // unreachable in task array
  TASK_BOLOS = 0,               // can call os
  TASK_SYSCALL,                 // can call os
  TASK_BOLOS_UX,                // can call os
  TASK_USERTASKS_START,
  TASK_USER_UX =
      TASK_USERTASKS_START, // must call syscalls to reach os, locked in ux ram
...
SYSCALL unsigned int os_sched_is_running(unsigned int task_idx);

/**
 * Retrieve the last status issued by a task using either yield or exit.
 */
SUDOCALL PERMISSION(APPLICATION_FLAG_NONE) unsigned int os_sched_last_status(
    unsigned int task_idx);
unsigned int svc_os_sched_last_status(unsigned int task_idx);
 ...
 */
SUDOCALL TASKSWITCH
    PERMISSION(APPLICATION_FLAG_NONE) void os_sched_yield(unsigned int status);
void svc_os_sched_yield(unsigned int status);
...
SUDOCALL TASKSWITCH PERMISSION(APPLICATION_FLAG_NONE) void os_sched_switch(
    unsigned int task_idx, unsigned int status);
void svc_os_sched_switch(unsigned int task_idx, unsigned int status);
...
SYSCALL void log_debug_int(char *fmt, unsigned int i);
SYSCALL void log_debug_int_nw(char *fmt, unsigned int i);
...
#define log_debug_int_nw(...)
#define trigger_gpio3(...)
#endif // TARGET_NANOX
#else  // BOLOS_DEBUG
#endif // BOLOS_DEBUG
